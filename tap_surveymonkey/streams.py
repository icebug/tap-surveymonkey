...
class PaginatedStream(Stream):
    def get_params(self, stream, config, state, bookmark_value):
        return {"per_page": int(config.get("page_size", DEFAULT_PAGE_SIZE)), "page": 1}
...
class Responses(PaginatedStream):
    ...
    def get_params(self, stream, config, state, bookmark_value):
        ...
        params.update({"sort_by": "date_modified", "sort_order": "ASC"})
    ...
    def _modify_record(self, raw_record):
        super()._modify_record(raw_record)
        for p in raw_record["pages"]:
            for q in p["questions"]:
                if q["id"] == "718337767":
                    for a in q["answers"]:
                        a["text"] = hashlib.md5(a["text"].encode()).hexdigest()
                if q["id"] in ("718337205", "718337856", "775051303"):
                    for a in q["answers"]:
                        a["text"] = "ANONYMIZED"
        raw_record["extraction_date"] = DATETIME_NOW
        patch_time_str(raw_record)
...
STREAMS = {
    "surveys": Surveys(stream_id="surveys", path="surveys"),
